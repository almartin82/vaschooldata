...
#' @examples
#' \dontrun{
#' wide_data <- fetch_enr(2023, tidy = FALSE)
...
tidy_enr <- function(df) {
  ...
  # Sex subgroups
  sex_cols <- c("male", "female")
  sex_cols <- sex_cols[sex_cols %in% names(df)]

  # Grade-level columns
  grade_cols <- grep("^grade_", names(df), value = TRUE)

  all_subgroups <- c(demo_cols, sex_cols)

  # Transform demographic/sex subgroups to long format
