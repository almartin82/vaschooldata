...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = va.get_available_years()
...
class TestFetchEnr:
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = va.fetch_enr(years['max_year'])
        # Total enrollment in Virginia varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Virginia should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = va.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = va.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pyvaschooldata as va
        years = get_test_years()
        df_single = va.fetch_enr(years['max_year'])
        df_multi = va.fetch_enr_multi([years['max_year']])
