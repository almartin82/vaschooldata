# ==============================================================================
# LIVE Pipeline Tests for vaschooldata
# ==============================================================================
#
...
test_that("Virginia DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Virginia DOE URL and verify HTTP 200")
})
...
test_that("Can download Virginia enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Virginia enrollment file", {
  skip_if_offline()
...
test_that("vaschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- vaschooldata::get_available_years()
  ...
  tryCatch({
    raw <- vaschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- vaschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- vaschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- vaschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- vaschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- vaschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- vaschooldata:::get_cache_path(2024, "enrollment")
